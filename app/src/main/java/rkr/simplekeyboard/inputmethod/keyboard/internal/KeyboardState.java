 ...
 * {@link #onPressKey(int,boolean,int,int)},
 * {@link #onReleaseKey(int,boolean,int,int)},
 * {@link #onEvent(Event,int,int)}, {@link #onFinishSlidingInput(int,int)},
 * {@link #onUpdateShiftState(int,int)},
 * {@link #onResetKeyboardStateToAlphabet(int,int)}.
 *
...
public final class KeyboardState {
	...
	// TODO: Merge {@link #mSwitchState}, {@link #mIsAlphabetMode}, {@link
	// #mAlphabetShiftState},
	...
	// Constants for {@link SavedKeyboardState#mShiftMode} and {@link
	// #setShifted(int)}.
	...
	public void onPressKey(final int code, final boolean isSinglePointer, final int autoCapsFlags,
		...
		if (code != Constants.CODE_SHIFT) {
			// Because the double tap shift key timer is to detect two consecutive shift key
			// press,
		...
		if (code == Constants.CODE_SHIFT) {
			...
			// It is required to reset the auto caps state when all of the following
			// conditions
			// are met:
			// 1) two or more fingers are in action
			// 2) in alphabet layout
			// 3) not in all characters caps mode
			// As for #3, please note that it's required to check even when the auto caps
			// mode is
			// off because, for example, we may be in the #1 state within the manual
			// temporary
			// shifted mode.
			if (!isSinglePointer && mIsAlphabetMode
					&& autoCapsFlags != TextUtils.CAP_MODE_CHARACTERS) {
				final boolean needsToResetAutoCaps = (mAlphabetShiftState.isAutomaticShifted() && !mShiftKeyState.isChording())
	...
	private void onReleaseSymbol(final boolean withSliding, final int autoCapsFlags,
		...
		if (mSymbolKeyState.isChording()) {
			// Switch back to the previous keyboard mode if the user chords the mode change
			// key and
			// another key, then releases the mode change key.
			toggleAlphabetAndSymbols(autoCapsFlags, recapitalizeMode);
		} else if (!withSliding) {
			// If the mode change key is being released without sliding, we should forget
			// the
			// previous symbols keyboard shift state and simply switch back to symbols
			// layout
	...
	private void updateAlphabetShiftState(final int autoCapsFlags, final int recapitalizeMode) {
		...
		if (!mShiftKeyState.isReleasing()) {
			// Ignore update shift state event while the shift key is being pressed
			// (including
			// chording).
			return;
		}
	...
	private void onReleaseShift(final boolean withSliding, final int autoCapsFlags,
		...
		if (RecapitalizeStatus.NOT_A_RECAPITALIZE_MODE != mRecapitalizeMode) {
			...
			if (mIsInDoubleTapShiftKey) {
				...
				// After chording input, automatic shift state may have been changed depending
				// on
			...
			// In symbol mode, switch back to the previous keyboard mode if the user chords
			// the
