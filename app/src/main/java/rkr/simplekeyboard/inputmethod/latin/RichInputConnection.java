...
public final class RichInputConnection {
	...
	public void deleteTextAfterCursor(final int numChars) {
		String textAfterCursor = mTextAfterCursor;
		if (!textAfterCursor.isEmpty() && textAfterCursor.length() >= numChars) {
			mTextAfterCursor = textAfterCursor.substring(numChars, textAfterCursor.length());
		}
		// if (mExpectedSelStart >= numChars) {
		// mExpectedSelStart -= numChars;
		// }

		mIC.deleteSurroundingText(0, numChars);
	}
